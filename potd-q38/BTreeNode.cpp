...
bool underflows(unsigned numElem, unsigned order) {
    // TODO: Your code here!
    if (numElem < std::ceil(order/2.0)-1)
        return true;
    else 
        return false;
}
...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
    ...
    int temp_children = prev->elements_.back();
    int temp_root = elements_[idx-1];
    children_[idx-1]->elements_.pop_back();
    elements_[idx-1] = temp_children;
    children_[idx]->elements_.insert(children_[idx]->elements_.begin(), temp_root);
    return true;
}
